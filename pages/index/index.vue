<template>
	<view>
		...
		<view class="main-bar flex-box">
			<view class="flex-item" v-for="(item,index) in contenrBar" :key="index">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 新碟新歌 
			getLatestAlbum() {
				apiGetTopAlbum().then(data => {
					console.log('data', data)
					// 所有的数据存储在latestTempAlbum
					this.latestTempAlbum = data.albums
					this.latestAlbum = data.albums.slice(0, 3)
				})
			},
			// 切换新碟新歌
			switchTab(tag) {
